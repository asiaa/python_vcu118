...
-- Homin 2019-0129 init. test for readout a dummy snap block from vcu118
'''

#TODO: add support for ADC histogram plotting.
#TODO: add support for determining ADC input level 

import casperfpga,casperfpga.snapadc,casperfpga.snap,time,numpy,struct,sys,logging,pylab,matplotlib
...
#    return acc_n, numpy.array(interleave_a,dtype=numpy.float) 
    return numpy.array(interleave_a,dtype=numpy.int) 
...
def main():
    ...
    logging.basicConfig(filename='homin_debug.log',level=logging.DEBUG)
...
#    print('Connecting to server %s on port %i... '%(snap,katcp_port)),
    print('Connecting to VCU118 server %s ... \n'%(vcu118)),

    fpga = casperfpga.CasperFpga(vcu118)
    time.sleep(0.2)
...
#        print 'Skipped.'
    ...
    # sampling clock. The init function will also tweak the alignment of the digital lanes th    # carry data from the ADC chips to the FPGA, to ensure reliable data capture. It should take about
    # 30 seconds to run.
...
#    adc = casperfpga.snapadc.SNAPADC(fpga, ref=10) # reference at 10MHz
    ...
    fpga.write_int(snap_name+'_ctrl',0,0)
    fpga.write_int(snap_name+'_ctrl',1,0)
    fpga.write_int(snap_name+'_ctrl',0,0)

#    fpga.write_int('snapshot_ctrl',1,0)
#    fpga.write_int('snapshot_ctrl',0,0)

    print fpga.read(snap_name+'_bram',100,0)
...
#    print interleave_a
    ...
    for i in range(snap_len):
        print ("%i %x")%(i,interleave_a[i])

#    snap = casperfpga.snap.Snap(fpga,'snapshot',32,0,1024)
